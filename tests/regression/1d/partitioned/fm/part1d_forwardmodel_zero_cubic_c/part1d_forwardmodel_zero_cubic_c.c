...
int main(int argc, char *argv[]) 
{
  ...
  results = part1d_forwardmodel_zero_cubic(burnin,
					   total,
					   min_part,
					   max_part,
					   xmin,
  ...
  if (results == NULL) {
    fprintf(stderr, 
	    "error: failed to run part1d_forwardmodel_zero_cubic_c\n");
    return -1;
  }
  ...
  if (rjmcmc_save_vector("part1d_forwardmodel_zero_cubic_c.data", data.sy, xsamples) < 0) {
  ...
  if (rjmcmc_save_vector("part1d_forwardmodel_zero_cubic_c.misfit", v, total) < 0) {
  ...
  if (rjmcmc_save_int_vector("part1d_forwardmodel_zero_cubic_c.partitions", iv, total) < 0) {
    fprintf(stderr, "error: failed to save partitions data\n");
    return -1;
  }
  if (rjmcmc_save_int_vector_as_histogram("part1d_forwardmodel_zero_cubic_c.partition_hist",
					  2,
					  max_part,
					  iv, total) < 0) {
  ...
  if (rjmcmc_save_int_coords("part1d_forwardmodel_zero_cubic_c.partition_x_hist",
			     xcoords,
			     iv,
			     xsamples) < 0) {
  ...
  if (rjmcmc_save_vector("part1d_forwardmodel_zero_cubic_c.mean", v, xsamples) < 0) {
