...
static double gaussian_misfit_hierarchical(void *user_arg,
				       int npartitions,
				       const double *partitions,
				       int nglobalparameters,
				       const double *global_parameters,
...
int main(int argc, char *argv[]) 
{
  ...
  if (rjmcmc_save_coords("gaussian.data", data.x, data.sy, xsamples) < 0) {
  ...
  if (rjmcmc_save_int_coords("gaussian.partition_x_hist",
			     xcoords,
			     iv,
			     xsamples) < 0) {
  ...
  if (rjmcmc_save_coords("gaussian.mean", xcoords, v, xsamples) < 0) {
  ...
  if (hierarchical) {
    ...
    if (rjmcmc_save_vector_as_histogram("gaussian.sigma_histogram",
...
static double fx(double x)
{
  ...
  return amp * exp(-(dx*dx)/(2.0*sigma*sigma));
}
			  ...
			  part1d_fm_value_at_t gradient_at)
{
  ...
  return gaussian_misfit_hierarchical(user_arg,
				  npartitions,
				  partitions,
				  nglobalparameters,
				  global_parameters,
...
static double
gaussian_misfit_hierarchical(void *user_arg,
			 int npartitions,
			 const double *partitions,
			 int nglobalparameters,
			 const double *global_parameters,
